...
3. **The Google Appointment Scheduling iframe URL comes from `env.APPOINTMENT_SCHEDULING_URL`**, not hardcoded. wrangler.toml already wires it; the renderer falls back to a "we'll email you a direct link" message if it's missing so dev environments without the var don't break. Cloudflare's preview/prod difference: the iframe loads fine on the real domain, but some local dev setups block the third-party frame — verify on the live preview URL, not just `astro dev`.
